...
// Shortens a long URL
exports.shortenUrl = () => {
  const output = 'http://dhsu.com/' + Math.random().toString(36).slice(2).substr(0, 7);
...
exports.find = (payload, err, success) => {
  db.urls.find({ where: ({ id: payload }) }).then(success).catch(err);
...
exports.update = (payload, err, success) => {
  db.urls.find({
    where: ({ id: payload.id }) }).then((currentData) => {
